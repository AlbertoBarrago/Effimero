...
RUN pnpm install --filter @margin/server --filter @margin/snippet --filter @margin/dashboard
...
COPY package.json pnpm-workspace.yaml pnpm-lock.yaml* ./
COPY packages/server/package.json packages/server/
RUN pnpm install --filter @margin/server --prod
COPY --from=build /app/packages/server/dist packages/server/dist
COPY --from=build /app/packages/dashboard/dist packages/server/public
COPY --from=build /app/packages/snippet/dist/margin.js packages/server/public/margin.js
EXPOSE 3000
